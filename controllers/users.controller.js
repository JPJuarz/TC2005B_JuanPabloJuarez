...
exports.post_login = (request, response, next) => {
    ...
    User.fetchOne(request.body.username).then(([rows, fieldData]) => {
        ...
        bcrypt.compare(request.body.password, rows[0][0].password).then((doMatch) => {  
            if (doMatch) {
                request.session.isLoggedIn = true;
                request.session.username = rows[0][0].username; 
                request.session.userId = rows[0][0].id;         

                User.fetchPermisos(rows[0][0].id).then(([permisos]) => { 
                    request.session.permisos = permisos[0].map(p => p.nombre); 
                    return request.session.save(() => {
